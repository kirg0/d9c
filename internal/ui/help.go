...
func (m Model) resourceKeyRows() []helpRow {
	...
	case ViewContainers:
		return []helpRow{
			...
			{":cp [<local> <ctr-dir>]", "Загрузить файл/каталог в контейнер (без аргументов — модалка с выбором файла)"},
	...
	case ViewImages:
		return []helpRow{
			...
			{k.Display(keymap.Select), "Отметить для массового удаления"},
			{"r", "Удалить выбранные образы (при выборе; с подтверждением)"},
